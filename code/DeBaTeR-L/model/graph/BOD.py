...
class TimeEncoder(nn.Module):
    def __init__(self, total_d, tdim, tlen):
...
class BOD(GraphRecommender):
    def __init__(self, conf, training_set, test_set,**kwargs):
        ...
        self.time_data = kwargs['temporal.data']
        self.timeEncoder = TimeEncoder(self.generator_emb_size, kwargs['temporal.tdim'], kwargs['temporal.tlen'])
        self.tmin = kwargs['temporal.tmin']
        self.testT = kwargs['temporal.test']
    ...
    def train(self):
        ...
        time_encoder = self.timeEncoder.cuda()
        optimizer_generator = torch.optim.Adam(list(model_generator.parameters())+list(time_encoder.parameters()), lr=self.generator_lr)
        ...
        for epoch_iter in range(self.maxEpoch):
            ...
            for inner_iter in range(self.inner_loop):
                ...
                for n, batch in enumerate(next_batch_pairwise(self.data, self.batch_size)):
                    ...
                    t_idx = [(int(self.data.id2user[u]),int(self.data.id2item[i])) for (u,i) in zip(u_idx, pos_i_idx)]
                    t_data = (torch.FloatTensor(itemgetter(*t_idx)(self.time_data)) - self.tmin).long().to('cuda')
                    t_emb = time_encoder(t_data)
                    
                    model.train()
                    rec_user_emb, rec_item_emb = model()
                    pos_user_emb_syn, pos_item_emb_syn = rec_user_emb[pos_u_idx] + t_emb, rec_item_emb[pos_i_idx] + t_emb
                    neg_user_emb_syn, neg_item_emb_syn = rec_user_emb[neg_u_idx] + t_emb, rec_item_emb[neg_i_idx] + t_emb

                    A_weight_user_item_full = model_generator(pos_user_emb_syn, pos_item_emb_syn, t_emb)
                    A_weight_user_item_full_neg = model_generator(neg_user_emb_syn, neg_item_emb_syn, t_emb)
            ...
            for ol_iter in range(self.outer_loop):
                ...
                t_idx = [(int(self.data.id2user[u]),int(self.data.id2item[i])) for (u,i) in zip(u_idx_ol, pos_i_idx_ol)]
                t_data = (torch.FloatTensor(itemgetter(*t_idx)(self.time_data)) - self.tmin).long().to('cuda')
                t_emb = time_encoder(t_data)
                
                rec_user_emb, rec_item_emb = model()
                user_emb_ol, item_emb_ol = rec_user_emb[u_idx_ol] + t_emb, rec_item_emb[i_idx_ol] + t_emb
                pos_user_emb_ol, pos_item_emb_ol, neg_item_emb_ol = rec_user_emb[pos_u_idx_ol] + t_emb, rec_item_emb[pos_i_idx_ol] + t_emb, rec_item_emb[neg_i_idx_ol] + t_emb

                A_weight_user_item_pos = model_generator(pos_user_emb_ol, pos_user_emb_ol, t_emb)
                A_weight_user_item_neg = model_generator(pos_user_emb_ol, neg_item_emb_ol, t_emb)
                ...
                A_weight_user_item = model_generator(user_emb_ol, item_emb_ol, t_emb)                
    ...
    def predict(self, u):
        with torch.no_grad():
            time = self.testT[int(u)]
            t_data = (torch.FloatTensor(time) - self.tmin).unsqueeze(0).long().to('cuda')
            t_emb = self.timeEncoder(t_data)
            u = self.data.get_user_id(u)
            score = torch.matmul(self.user_emb[u]+t_emb, (self.item_emb+t_emb).transpose(0, 1))
...
class GraphGenerator_VAE(nn.Module):
    def __init__(self, data, emb_size):
        ...
        self.data = data
        self.latent_size = emb_size
        self.encoder = nn.Linear(self.latent_size*3, 64)
    ...
    def forward(self, user_e, item_e, time_e):
        input_vec = torch.cat((user_e,item_e, time_e), axis = 1)
